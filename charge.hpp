...
class Charge : public sf::CircleShape{
  ...
  public:
    ...
    Charge(float radius=0, std::size_t pointCount=30, float charge=0.f, float mass=1.f, sf::Vector2f velocity=sf::Vector2f(0.f,0.f)) :
    CircleShape(radius, pointCount), charge(charge), velocity(velocity), mass(mass), id(global_id++), force(sf::Vector2f(0.f,0.f)){}
    const uint32_t id;
