...
int main()
{
    ...
    sf::Text chargeCount("Charges: " + std::to_string(chargeCount_int),courier_prime, 30);
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
                ...
                if (event.mouseButton.button == sf::Mouse::Left)
                {
                    ...
                    if (tool == force)
                    {
                      for (auto chargePtr : charge_vector)
                      {
                        ...
                        else forceCharge = nullptr;
                      }
                      if (forceCharge != nullptr)
                      {
                        force_line.setVertexPosition((*forceCharge).getPosition(), 0);
                      }
                    }
                    else if (tool == charge)
                    {
                    ...
                    else if (tool == trash)
                    {
                        for (auto it = charge_vector.begin(); it != charge_vector.end(); it++)
                        {
                          if ((*it)->getIsCursorOn())
                          {
                            ...
                            chargeCount.setString("Charges: " + std::to_string(charge_vector.size()));
                            break;
                          }
                        }
                    }
            ...
            if (event.type == sf::Event::KeyPressed)
            {
                switch(event.key.code)
                {
                    ...
                    //case sf::Keyboard::L: lockView = !lockView; break;
                }
            }
        ...
        if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
        {
          ...
          {
            force_line.setVerticesPosition((*forceCharge).getPosition(), mousePos);
